...
command -v fpm >/dev/null || die "fpm is required to build deb packages"

mkdir -p "$OUTPUT_DIR"
STAGING_DIR=$(mktemp -d "$OUTPUT_DIR/deb-root.XXXXXX")
...
fpm \
    ...
    --deb-recommends "gpu-screen-recorder" \
    --deb-suggests "gpu-screen-recorder-notification" \
    --depends "sqlite3" \
...
find "$OUTPUT_DIR" -maxdepth 1 -name "*.deb" -print | sort | tail -n 1
