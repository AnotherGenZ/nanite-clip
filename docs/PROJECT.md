...
Define the clip rule model and the evaluation engine. This layer should stay pure and deterministic as much as possible so it remains straightforward to test.
...
- New trigger types: extend the rule model and engine, then expose configuration in the UI.
...
This project should remain a focused local automation tool: detect meaningful PlanetSide 2 moments, preserve the relevant replay footage, and stay dependable on a single user's machine.
